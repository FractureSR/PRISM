    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        ...
        with accelerator.accumulate(model):
            ...
            for forward_idx in range(args.forward_num_total): ### forward for multiple times
                predict = model(hidden_states, input_ids, attention_mask, q_hidden_states=q_hidden_states) ### for me. just enable the model to switch paratmers is enough
        ...
        for batch_idx, data in enumerate(tqdm(test_loader)):
            ...
            with torch.no_grad():
                ...
                for forward_idx in range(args.forward_num_total):
                    predict = model(data["hidden_states"], input_ids=data["input_ids"], attention_mask=data["attention_mask"], q_hidden_states=q_hidden_states)
                    if q_hidden_states is None:
