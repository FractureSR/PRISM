...
def repeat_kv(hidden_states: torch.Tensor, n_rep: int) -> torch.Tensor:
    ...
    """
    batch, num_key_value_heads, slen, head_dim = hidden_states[0].shape
    if n_rep == 1:
        return hidden_states
    hidden_states = hidden_states[:, :, :, None, :, :].expand(-1, batch, num_key_value_heads, n_rep, slen, head_dim)
    return hidden_states.reshape(-1, batch, num_key_value_heads * n_rep, slen, head_dim)
...
class LlamaAttention(nn.Module):
    ...
    def forward(
            ...
            q_hidden_states: Optional[torch.Tensor] = None,
    ) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
        bsz, q_len, _ = hidden_states.size()

        if self.config.pretraining_tp > 1: ### can't enter this
            ...
            forward_num = 1 if q_hidden_states is None else q_hidden_states.shape[0] + 1 ### defines forward num
            ...
            if forward_num > 1:
                ...
                all_hidden_states = hidden_states[None, :, :, :]
        ...
        past_key_value = (key_states[-1], value_states[-1]) if use_cache else None
        ...
        for idx in range(forward_num):
            ...
            if not idx:
                ...
                align_small_mask = torch.eye(q_len - forward_num + idx + 1)
                
            align_big_mask = torch.zeros((q_len, q_len))
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        residual = hidden_states if q_hidden_states is None else q_hidden_states[-1]
        ### inside the decoder
        ### use hidden states if there is no q_hidden_states. typically for the first forward
        ### if there is q_hidden_states, use the last one to be the decoder residual

        if self.index != 0:
            hidden_states = self.input_layernorm(hidden_states)

        # Self Attention
        ### this attn is adapted to tolerate q_hidden_states
...
class Model(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.fc(torch.cat((inputs_embeds, hidden_states), dim=-1)) ### hidden_states is already fusioned embed and feature
        ...
        ### if q_hidden_states is not None, replicate input_embeds q_hidden_states.shape[0] times
        ### cat expanded input_embeds and q_hidden_states along the last dim, for fusion
        ### after that, each feature will be a fusioned feature
        ### q_hidden_states is a substitute for hidden_states
